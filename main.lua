...
ghostPart = ghostBlock -- Referans düzeltme

-- E TUŞU: 5 SANİYE FULL+10 LAG
UIS.InputBegan:Connect(function(input, gpe)
    if not gpe and input.KeyCode == Enum.KeyCode.E and antiAttackActive then
        e_active = true
        task.wait(5) -- İstediğin 5 saniye
        e_active = false
    end
end)

RunService.Heartbeat:Connect(function()
    -- Siyah-Beyaz Ekran Engelleyici (Always On)
    if Lighting:FindFirstChildOfClass("ColorCorrectionEffect") then
        Lighting:FindFirstChildOfClass("ColorCorrectionEffect").Saturation = 0
    end
    ...
    -- 1. EFSANEVİ HYPER SPEED (SADECE TUŞA BAĞLI)
    if hyperActive and hum.MoveDirection.Magnitude > 0 then
        hrp.CFrame = hrp.CFrame + (hum.MoveDirection * hyperMultiplier)
        hrp.Velocity = Vector3.new(hrp.Velocity.X, 0, hrp.Velocity.Z)
    end

    -- 2. UPGRADED ANTI-ATTACK
    if antiAttackActive then
        -- Katil "Öldür" Butonunu Kapatma (Prompt Bypass)
        ...
        -- Senin Ölümsüzlük Kodun (FULL+ UPGRADE)
        hum.MaxHealth = math.huge
        hum.Health = math.huge
        hum.BreakJointsOnDeath = false
        hum:SetStateEnabled(Enum.HumanoidStateType.Dead, false)

        -- Havalı Yeşil Saydam Görünüm (Sabit Blok)
        ...
        -- LAG SİSTEMİ (E'YE BASINCA 5 SANİYE KASIRGA)
        for _, enemy in pairs(Players:GetPlayers()) do
            if enemy ~= player and enemy.Character and enemy.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if dist < 25 then
                    if e_active then
                        -- E basınca 5 saniye Full+10 Lag
                        eHrp.CFrame = eHrp.CFrame * CFrame.Angles(0, math.rad(120), 0)
                        eHrp.Velocity = Vector3.new(0, -2000, 0)
                    else
                        -- Pasif halde hafif dönme (Karakterin dönmemesi için soft)
...
-- 80X MEGA REGEN (Sürekli Can Pompalama)
for i = 1, 80 do
    task.spawn(function()
        while true do
